...
class Model(nn.Module):
    def __init__(self, featmatrix1=None, featmatrix2=None) -> None:
        ...
        self.lstm = nn.LSTM(72, 32, batch_first=True, bidirectional=False)
        ...
        if not featmatrix1 and not featmatrix2:
...
model = Model(featmatrix1, featmatrix2)
def lr_lambda(epoch):
    if epoch > 35:
        return 0.02
    if epoch > 20:
...
# for i, (train_idx, test_idx) in enumerate(StratifiedKFold(shuffle=True, random_state=2022).split(train_set[['sn', 'fault_time','feature', 'servertype', 'label']], train_set['label'])):
#     train_set_ = train_set.iloc[train_idx]
#     train_set_ = pd.concat([train_set_, train_set_[train_set_.label==0]]).reset_index(drop=True)
#     test_set_ = train_set.iloc[test_idx]     
#     train_and_evaluate(train_set_, test_set_, submit_df, i)
