...
venus_log_test_finala = pd.read_csv('/tcdata/final_venus_dataset_a.csv')
...
def venus_process(fault_time, venus_time, module, module_cause):
    ...
    for modu in module.split(','):
        ...
        if 'module' in modu:
            log = module_cause[index: index+4]
            log = [venus_dict.get(l, 1) for l in log]
        else:
            log = module_cause[index: index+2]
            log = [venus_dict.get(l, 1) for l in log] + [0, 0]
        venus_seq.append(log) 
    return  json.dumps(venus_seq[-10:])
...
test_df_a['venus_feature'] = json.dumps([[0]*4])
test_df_b['venus_feature'] = json.dumps([[0]*4])
